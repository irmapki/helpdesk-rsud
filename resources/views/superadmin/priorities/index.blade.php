<x-app-layout>
    <div class="space-y-6">
        <!-- Top Title -->
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Prioritas Tiket &amp; Konfigurasi SLA</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-6 space-y-4">
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-xl bg-orange-100 text-orange-700 flex items-center justify-center font-bold text-sm">
                        +
                    </div>
                    <div>
                ...
                <form action="{{ route('superadmin.priorities.store') }}" method="POST" class="space-y-4 text-xs">
                    ...
                    <div>
                        ...
                        @error('sla_hours') <p class="text-[11px] text-rose-500 mt-1">{{ $message }}</p> @enderror
                    </div>
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-6 overflow-hidden">
                <h3 class="font-bold text-base text-slate-900 mb-4">Daftar Konfigurasi SLA &amp; Prioritas</h3>
                            ...
                            @forelse ($priorities as $pri)
                                <tr x-data="{ edit: false }" class="hover:bg-slate-50/80 transition">
                                    <td class="py-3.5 px-2">
                                        ...
                                        <div x-show="edit">
                                            <input type="text" form="edit-pri-form-{{ $pri->id }}" name="name" value="{{ $pri->name }}" class="text-xs rounded-xl border-slate-200 bg-white text-slate-800 p-1.5 w-full">
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-2">
                                        ...
                                        <div x-show="edit">
                                            <input type="number" form="edit-pri-form-{{ $pri->id }}" name="sla_hours" value="{{ $pri->sla_hours }}" class="text-xs rounded-xl border-slate-200 bg-white text-slate-800 p-1.5 w-24">
                                    ...
                                    <td class="py-3.5 px-2 text-slate-500">
                                        ...
                                        <div x-show="edit">
                                            <input type="text" form="edit-pri-form-{{ $pri->id }}" name="description" value="{{ $pri->description }}" class="text-xs rounded-xl border-slate-200 bg-white text-slate-800 p-1.5 w-full">
                                        </div>
                                    </td>
