<x-app-layout>
    ...
    <x-slot name="breadcrumb">
        Super Admin &rsaquo; Manajemen Pengguna &rsaquo; Edit: {{ $user->name }}
    </x-slot>

    <div class="p-6 lg:p-8 space-y-6 max-w-[1000px] mx-auto">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Data Pengguna</h1>
                <p class="text-xs text-slate-500 mt-1">Perbarui profil, hak akses role, unit penempatan, atau ganti kata sandi.</p>
            </div>
            ...
            <form action="{{ route('superadmin.users.update', $user) }}" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="password" class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500" placeholder="Minimal 8 karakter baru">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="role_id" id="role_id" required class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                            @foreach ($roles as $r)
                    ...
                    <div>
                        ...
                        <select name="unit_id" id="unit_id" class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                ...
                <div>
                    ...
                    <input type="text" name="specialization" id="specialization" value="{{ old('specialization', $user->specialization) }}" class="w-full text-xs rounded-xl border-slate-200 focus:ring-teal-500 focus:border-teal-500">
                </div>
                ...
                <div class="pt-2">
                    <label class="inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-2 text-xs font-bold text-slate-700">Akun Aktif (Dapat login)</span>
                    </label>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="bg-[#0a252a] hover:bg-[#0e353c] text-white font-bold text-xs px-6 py-2.5 rounded-xl shadow-xs transition">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
