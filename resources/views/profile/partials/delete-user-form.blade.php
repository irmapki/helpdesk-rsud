<section class="space-y-6">
    ...
    <x-danger-button
        ...
        class="bg-red-600 hover:bg-red-700 active:bg-red-800 focus:ring-red-500 rounded-xl px-5 py-2.5"
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6 bg-white rounded-2xl">
            ...
            <div class="mt-6 flex justify-end gap-3">
                <x-secondary-button x-on:click="$dispatch('close')" class="rounded-xl px-4 py-2 border-gray-300 text-gray-700">
                    {{ __('Batal') }}
                </x-secondary-button>

                <x-danger-button class="bg-red-600 hover:bg-red-700 active:bg-red-800 focus:ring-red-500 rounded-xl px-5 py-2.5">
                    {{ __('Hapus Akun') }}
                </x-danger-button>
            </div>
        </form>
