...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f8fafc; color: #1e293b; margin: 0; padding: 20px;">
    <div style="max-width: 580px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
        ...
        <div style="padding: 28px;">
            <div style="display: inline-block; background-color: #ecfdf5; color: #047857; font-size: 11px; font-weight: 800; padding: 4px 12px; rounded: 8px; border-radius: 6px; text-transform: uppercase; margin-bottom: 12px;">
                Tugas Baru Teknisi
            </div>

            <h2 style="margin: 0 0 10px 0; font-size: 16px; color: #0f172a;">Halo, {{ $ticket->technician->name ?? 'Teknisi IT' }}! 👋</h2>
            <p style="margin: 0 0 20px 0; font-size: 13px; color: #475569; line-height: 1.5;">
                Admin Helpdesk telah menugaskan Anda untuk menangani pengaduan kendala teknis berikut:
            </p>
            ...
            <table style="width: 100%; border-collapse: separate; border-spacing: 0; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; margin-bottom: 24px; font-size: 13px;">
                ...
                <tr>
                    <td style="padding: 10px 14px; color: #64748b; font-weight: bold; border-bottom: 1px solid #e2e8f0;">Unit / Lokasi</td>
                    <td style="padding: 10px 14px; font-weight: 600; color: #334155; border-bottom: 1px solid #e2e8f0;">{{ $ticket->unit->name ?? '-' }}</td>
                </tr>
                <tr>
                    <td style="padding: 10px 14px; color: #64748b; font-weight: bold; border-bottom: 1px solid #e2e8f0;">Kategori</td>
                    <td style="padding: 10px 14px; font-weight: 600; color: #334155; border-bottom: 1px solid #e2e8f0;">{{ $ticket->category->name ?? '-' }}</td>
                </tr>
                <tr>
                    <td style="padding: 10px 14px; color: #64748b; font-weight: bold;">Prioritas & SLA</td>
                    <td style="padding: 10px 14px; font-weight: 700; color: #b91c1c;">{{ $ticket->priority->name ?? 'Normal' }}</td>
                </tr>
            </table>

            @if($ticket->admin_notes)
                <div style="background-color: #fffbeb; border: 1px solid #fef3c7; border-left: 4px solid #f59e0b; padding: 12px; border-radius: 8px; margin-bottom: 24px; font-size: 12px; color: #92400e;">
            ...
            <div style="text-align: center; margin: 30px 0 10px 0;">
                <a href="{{ route('teknisi.dashboard', ['ticket_id' => $ticket->id]) }}" 
                   style="display: inline-block; background-color: #0f766e; color: #ffffff; text-decoration: none; padding: 12px 28px; border-radius: 12px; font-weight: 800; font-size: 13px; box-shadow: 0 4px 10px rgba(15, 118, 110, 0.3);">
                    Buka Workboard & Mulai Penanganan &rarr;
                </a>
            </div>
        </div>
        ...
        <div style="background-color: #f8fafc; padding: 16px; text-align: center; font-size: 11px; color: #94a3b8; border-top: 1px solid #e2e8f0;">
            Email ini dikirim otomatis oleh Sistem Helpdesk RSUD RAA Soewondo. Harap tidak membalas email ini.
        </div>
    </div>
</body>
</html>