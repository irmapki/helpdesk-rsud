...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    ...
    <body class="font-sans antialiased bg-[#f8fafc] text-slate-800 h-full overflow-hidden">
        <!-- Alpine.js Responsive State (Zero-Flash Page Navigation + Persisted State) -->
        <div x-data="{
        ...
        class="flex h-screen overflow-hidden relative w-full bg-[#f8fafc]">
            ...
            <!-- Mobile Backdrop (Overlay gelap ketika sidebar dibuka di layar HP/Tablet) -->
            ...
            <!-- SIDEBAR UTAMA (Gaya Gemini/ChatGPT: Icon Rail saat Collapsed + Full Drawer saat Expanded) -->
            <aside :class="{
                ...
                <div class="relative z-10 flex-1 flex flex-col min-h-0">
                    ...
                    <div class="h-16 sm:h-20 px-3.5 sm:px-4 border-b border-white/10 flex items-center justify-between shrink-0">
                        ...
                        <div x-show="desktopExpanded || !isDesktop" class="flex items-center justify-between w-full min-w-0">
                            ...
                            <!-- Tombol Ciutkan (Desktop) / Tutup (Mobile) -->
                        ...
                        <!-- 2. Tampilan Header Mode Icon Rail (Collapsed di Desktop) -->
                    ...
                    <!-- Area Navigasi Menu Utama Berdasarkan Role -->
                            ...
                            <div>
                                ...
                                <nav class="space-y-1.5">
                                    ...
                                    <a href="{{ route('superadmin.dashboard') }}"
                                        ...
                                        {{-- Tooltip Saat Collapsed di Desktop --}}
                                        <div x-show="!desktopExpanded && isDesktop" 
                                            class="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-slate-900 text-white text-xs font-semibold rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-2xl z-50 border border-slate-700">
                                            Dashboard Super Admin
                                            <div class="absolute -left-1 top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-900"></div>
                                        </div>
                                    </a>
                        ...
                        @if(auth()->check() && (auth()->user()->hasRole('admin') || auth()->user()->role?->name === 'admin'))
                            <div>
                                ...
                                <nav class="space-y-1.5">
                                    ...
                                    {{-- 2. Manajemen & Triage Tiket --}}
                                    <a href="{{ route('admin.tickets.index') }}"
                                        @click="closeMobile()"
                                        title="Manajemen & Triage Tiket"
                                        ...
                                        :class="{ 'justify-center px-0': !desktopExpanded && isDesktop }">
                                        <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('admin.tickets.*') ? 'text-emerald-800' : 'text-teal-300 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012-2m-6 9l2 2 4-4"></path>
                                        </svg>
                                        <span class="truncate" x-show="desktopExpanded || !isDesktop">Manajemen & Triage</span>

                                        <div x-show="!desktopExpanded && isDesktop" 
                                            class="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-slate-900 text-white text-xs font-semibold rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-2xl z-50 border border-slate-700">
                                            Manajemen & Triage Tiket
                                            <div class="absolute -left-1 top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-900"></div>
                                        </div>
                                    </a>
                                </nav>
                            ...
                            <div>
                                ...
                                <nav class="space-y-1.5">
                                    ...
                                    <a href="{{ route('teknisi.dashboard') }}"
                                        ...
                                        <div x-show="!desktopExpanded && isDesktop" 
                                            class="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-slate-900 text-white text-xs font-semibold rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-2xl z-50 border border-slate-700">
                                            Tiket Saya & Workboard
                                            <div class="absolute -left-1 top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-900"></div>
                                        </div>
                                    </a>

                                    {{-- 2. Riwayat Penanganan --}}
                    ...
                    <!-- Footer Sidebar: Logout Button & User Compact Tooltip -->
                    <div class="p-3 sm:p-4 border-t border-white/10 shrink-0">
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <button type="submit" 
                                ...
                                <div x-show="!desktopExpanded && isDesktop" 
                                    class="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-slate-900 text-rose-300 text-xs font-semibold rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-2xl z-50 border border-slate-700">
                                    Keluar Sistem
                                    <div class="absolute -left-1 top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-900"></div>
                                </div>
                            </button>
            ...
            <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#f8fafc]">
                ...
                <header class="bg-white shadow-xs z-20 flex items-center justify-between h-16 sm:h-20 px-4 sm:px-8 border-b border-slate-200/80 flex-shrink-0 gap-2">
                    <div class="flex items-center gap-2.5 sm:gap-4 min-w-0">
                        ...
                        <!-- Tombol Toggle Sidebar (Gemini / ChatGPT Style Hamburger) -->
                        <button @click="toggleSidebar()" 
