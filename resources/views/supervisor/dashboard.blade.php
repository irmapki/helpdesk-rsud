    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                ...
                <div class="text-3xl font-black text-slate-900">{{ $totalTicketsMonth ?? 42 }}</div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                ...
                <div class="text-3xl font-black text-amber-600">{{ $pendingTicketsCount ?? 3 }}</div>
                <span class="text-[11px] font-bold text-amber-600 mt-1 inline-block">Perlu pengawasan</span>
            </div>

            <div class="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                <span class="text-xs font-semibold text-slate-500 block mb-1">Penyelesaian Sesuai SLA</span>
                <div class="text-3xl font-black text-emerald-600">{{ $slaCompliance ?? 96.4 }}%</div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between">
                ...
                <div class="text-3xl font-black text-sky-700">{{ $activeTechCount ?? 5 }} / {{ $totalTechCount ?? 5 }}</div>
                <span class="text-[11px] font-bold text-sky-600 mt-1 inline-block">Divisi IT Standby</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-8 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-6 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
                    <div>
                        <h2 class="text-base font-bold text-slate-900">Monitoring Eskalasi &amp; Tiket Kritis</h2>
                        <p class="text-xs text-slate-500 mt-0.5">Ditinjau dan diawasi langsung oleh Supervisor IT</p>
                    </div>

                    <a href="{{ route('supervisor.monitoring-sla') }}" class="text-xs font-bold text-emerald-800 hover:text-emerald-950 bg-emerald-50 px-3.5 py-2 rounded-xl border border-emerald-200 transition">
                    ...
                    @forelse ($criticalTickets as $t)
                        <div class="border border-slate-200 hover:border-emerald-600 rounded-2xl p-4 bg-white transition shadow-xs">
                            <div class="flex justify-between items-start mb-1.5">
                                ...
                                <span class="text-xs font-bold {{ $t->sla_status === 'breached' ? 'text-rose-600' : ($t->sla_status === 'approaching' ? 'text-amber-600' : 'text-emerald-700') }}">
                                    {{ $t->status_label }}
                                </span>
                            </div>
            ...
            <div class="lg:col-span-4 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-6 space-y-4">
                <div>
                    <h3 class="text-sm font-bold text-slate-900">Beban Kerja Teknisi</h3>
                    ...
                    @forelse ($technicians as $tech)
                        ...
                        <div class="bg-slate-50 p-4 rounded-xl border border-slate-150">
                            <div class="flex justify-between mb-2 font-bold">
                                ...
                                <span class="text-emerald-800 font-bold">{{ $activeCount }} Tiket Aktif</span>
                            </div>
